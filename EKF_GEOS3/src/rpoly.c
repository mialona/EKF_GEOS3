/** @file rpoly.c
 *  @brief Find roots of a polynomial with real coefficients.
 *
 *  This driver contains the code for the computation of roots
...
int real_poly_roots(double *p, int degree,
                                  double **zeror, double **zeroi)
{
    struct RPoly_State *state = real_poly_alloc(degree);
    *zeror = v_create(degree);
    *zeroi = v_create(degree);
